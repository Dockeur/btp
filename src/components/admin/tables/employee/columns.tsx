...
export const getColumns = ({
    ...
    options
        ...
        {
            title: <HeaderCell title="Profession" sortable
                ascending={sortConfig?.direction === "asc" && sortConfig?.key === "position"} />,
            dataIndex: "position",
            key: "position",
            onHeaderCell: () => onHeaderCellClick("position"),
            render: (position: any, row: any) => (
                <div>
                    <Text className="!text-sm whitespace-nowrap font-medium">
                        {position}
                    </Text>
                </div>
            ),
        ...
        },
        {
            ...
            render: (_: any, row: any) => (
                <div className="inline-flex gap-3">
                    <Link to={routes.admin.employees.show(row.id)}>
                        <ActionIcon variant="outline" className="cursor-pointer dark:border-slate-600">
                            <FiEye className="h-4 w-4" />
                        </ActionIcon>
                    </Link>
                    <Link to={routes.admin.employees.edit(row.id)}>
                        <ActionIcon variant="outline" className="cursor-pointer dark:border-slate-600">
