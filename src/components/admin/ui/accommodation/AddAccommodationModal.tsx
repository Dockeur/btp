...
const AddAccommodationModal = ({ properties, modalState, setModalState, refresh, setRefresh, property_id = "" }: any) => {
    ...
    const { register, control, reset, handleSubmit, formState: { errors } } = useForm<AddAccommodationSchema>({
    ...
    const onSubmit = async (data: AddAccommodationSchema, e: any) => {
        try {
            ...
            toast.promise(createAccommodation(formData), {
                pending: "Création d'un logement...",
                success: {
                    render() {
                        setLoading(false);
                        ...
                        setRefresh(!refresh);
                        reset();
                        return "Logement crée avec succès"
                    }
                },
                error: {
                    render(error: any) {
            ...
            <Modal isOpen={modalState.open && modalState.id === 0 && modalState.type === "create"} onClose={() => setModalState({ id: 0, open: false, type: "create" })}>
                <div className="flex flex-col gap-4 p-4 dark:bg-neutral-900 dark:text-white rounded-lg bg-white">
                    <div className="flex items-center justify-between">
                        <Text className='text-2xl font-semibold'>Créer un logement</Text>
                        <ActionIcon
                            size="sm"
                            variant="text"
