...
const UpdateProductModal = ({ data, product, type, setType, modalState, setModalState, refresh, setRefresh }: any) => {
    const [loading, setLoading] = useState(false);
    const { handleError, getError } = useAuth()
    const { register, control, handleSubmit, formState: { errors } } = useForm<ProductSchema>({
        resolver: zodResolver(productSchema),
        values: {
            type: product.productable_type.split("\\")[2].toLowerCase(),
            status: product.status,
            publish: product.published_at ? true : false,
            productable_id: String(product.productable_id),
    ...
    const onSubmit = async (data: ProductSchema) => {
        try {
            ...
            toast.promise(updateProduct(data, product.id), {
                pending: "Modification d'un produit...",
                success: {
                    render() {
                        setLoading(false);
                        setModalState({ open: false, id: product.id, type: "edit" });
                        setRefresh(!refresh);
    ...
    return (
        <>
            <Modal isOpen={modalState.open && modalState.id === product.id && modalState.type === "edit"} onClose={() => setModalState({ id: product.id, open: false, type: "edit" })}>
                <div className="flex flex-col gap-4 p-4 dark:bg-neutral-900 dark:text-white rounded-lg bg-white">
                    <div className="flex items-center justify-between">
                        ...
                        <ActionIcon
                            size="sm"
                            variant="text"
                            onClick={() => setModalState({ id: product.id, open: false, type: "edit" })}
                        >
