...
const AddRoleModal = ({ permissions, modalState, setModalState, refresh, setRefresh }: any) => {
    ...
    const { register, reset, control, handleSubmit, formState: { errors } } = useForm<z.infer<typeof schema>>({
    ...
    const onSubmit = async (data: z.infer<typeof schema>) => {
        try {
            ...
            toast.promise(createRole(data), {
                pending: "Création d'un role...",
                success: {
                    render(data: any) {
                        setLoading(false);
                        setModalState({ open: false, id: 0 });
                        setRefresh(!refresh);
                        reset();
                        return "Role crée avec succès"
                    }
                },
                error: {
                    render(error: any) {
    ...
    return (
        <>
            <Modal isOpen={modalState.open && modalState.id === 0} onClose={() => setModalState({ open: false, id: 0 })}>
                <div className="flex flex-col gap-4 p-4 dark:bg-neutral-900 dark:text-white rounded-lg bg-white">
                    <div className="flex items-center justify-between">
                        <Text className='text-2xl font-semibold'>Créer un role</Text>
                        <ActionIcon
                            size="sm"
                            variant="text"
                            onClick={() => setModalState({ open: false, id: 0 })}
                        >
